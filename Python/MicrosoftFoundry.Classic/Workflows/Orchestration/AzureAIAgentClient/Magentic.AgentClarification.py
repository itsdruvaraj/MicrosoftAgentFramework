...
Thread Management:
- For Magentic workflows, thread management is handled internally by the workflow.
- For direct agent invocations (outside of workflows), you can pass thread_id to
  AzureAIAgentClient to maintain conversation context across multiple calls:
  
    client = AzureAIAgentClient(credential=credential, thread_id="your-thread-id")
  
  Or use agent.get_new_thread() to create a new thread:
  
    thread = agent.get_new_thread()
    result = await agent.run("your task", thread=thread)
...
Prerequisites:
- Azure AI credentials configured for `AzureAIAgentClient`.
...
async def main() -> None:
    async with DefaultAzureCredential() as credential:
        # Create AzureAIAgentClient instances for each agent
        # Note: For Magentic workflows, thread management is handled internally by the workflow.
        # Passing explicit thread_ids can cause conflicts with the workflow's state management.
        # For direct agent invocations (outside of workflows), you can pass thread_id to maintain
        # conversation context: AzureAIAgentClient(credential=credential, thread_id="your-thread-id")
        onboarding_client = AzureAIAgentClient(credential=credential)
