...
def create_catalog_agent(client: AzureAIAgentClient) -> ChatAgent:
    return ChatAgent(
        name="CatalogAgent",
...
def create_order_agent(client: AzureAIAgentClient) -> ChatAgent:
    return ChatAgent(
        name="OrderAgent",
...
def create_invoice_agent(client: AzureAIAgentClient) -> ChatAgent:
    return ChatAgent(
        name="InvoiceAgent",
...
async def main() -> None:
    ...
    async with DefaultAzureCredential() as credential:
        # Create clients that we'll manage lifecycle for
        catalog_client = AzureAIAgentClient(credential=credential, should_cleanup_agent=False)
        order_client = AzureAIAgentClient(credential=credential, should_cleanup_agent=False)
        invoice_client = AzureAIAgentClient(credential=credential, should_cleanup_agent=False)
        ...
        try:
            ...
            builder = SequentialBuilder().register_participants([
                ...
                lambda: create_catalog_agent(catalog_client),
                lambda: create_order_agent(order_client),
                lambda: create_invoice_agent(invoice_client),
            ])
